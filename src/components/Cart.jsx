...
const Cart = ({ myData, setUpdatedData, confirmOrder }) => {
  const anyOrder = myData.some((item) => item.order);
  const totalOrder = myData.reduce((total, item) => {
    return total + item.order;
  }, 0);
  ...
  return (
    <div>
      <div className="col-span-1 bg-white h-auto rounded-lg p-8 place-self-center">
        <h1 className="text-2xl text-Red font-bold">
          Your Cart ({totalOrder})
        </h1>
        {anyOrder ? (
          <div>
            ...
            <div className="flex gap-2 bg-Rose-50 justify-center items-center rounded-lg py-4">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="21"
                height="20"
